...
def create_pokemon_html(pokemon):
    ...
    sprite_url = f"https://remiscan.fr/shinydex/images/pokemon-sprites/webp/512/poke_capture_{dex:04d}_000_mf_n_00000000_f_n.webp"
    sprite_s_url = f"https://remiscan.fr/shinydex/images/pokemon-sprites/webp/512/poke_capture_{dex:04d}_000_mf_n_00000000_f_r.webp"
    ...
    with open(f"{pokemon.name.capitalize()}.html", "w") as f:
        ...
        try:
            # Se base_stats for um objeto com atributos diretamente acessíveis:
...
# Processar Pokémon Alola e gerar HTML para cada um
alola_pokemon_list = []  # Lista para armazenar os Pokémon Alola

# Aqui o código percorreria os Pokémon de dex 722 até 808 (Alola)
for dex in range(722, 809):
    try:
        pokemon = pypokedex.get(dex=dex)  # Obter Pokémon pela dex
        alola_pokemon_list.append(pokemon)  # Adicionar à lista

        # Criar o HTML do Pokémon
        create_pokemon_html(pokemon)

        # Aqui poderiam existir funções adicionais para atualizar catálogos ou fazer mais processamento
        # Exemplo de função que você poderia ter definido:
...
# Processar Pokémon de Alola (caso necessário)
# Defina a função `process_alola_pokemons` de acordo com sua necessidade
